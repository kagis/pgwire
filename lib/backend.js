...
exports.PGBackendMsgStream = class PGBackendMsgStream extends Transform {
  constructor() {
  ...
  switch (ident) {
    case 'R':
      switch (payload.read_i32be()) {
        ...
        default: throw new Error('Unknown backend auth message');
      }
    case 'S': return {
      tag: 'ParameterStatus',
...
function read_error_or_notice(payload) {
  ...
  return {
    ...
    internal_position: fields['p'] && +fields['p'],
    internal_query: fields['q'],
    where: fields['W'],
    file: fields['F'],
    line: fields['L'] && +fields['L'],
    routine: fields['R'],
