...
async function logicalReplication(conn, {
  ...
  return new ReplicationStream({ rx, tx, startLsn, ackIntervalMillis, highWaterMark });
}

class ReplicationStream extends Readable {
  constructor({ rx, tx, startLsn, ackIntervalMillis, highWaterMark }) {
  ...
  ack(lsn) {
    // TODO force 00000000/00000000 format before compare
