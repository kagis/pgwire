import BufferReader from './bufreader.js';
import { Transform } from 'stream';

export function read_replication_message(buf) {
...
export function read_pgoutput_message(buf) {
  const bufr = new BufferReader(buf);
  switch (bufr.read_char()) {
    case 'I': return {
      tag: 'insert',
...
export function replication_update_status_msg(lsn) {
...
export function replication_stream() {
  return new ReplicationStream();
}

export class ReplicationStream extends Transform {
  constructor() {
    super({
      objectMode: true,
      highWaterMark: 1000,
...
export function lsn_buf2hex(buf) {
  return (
...
export function lsn_hex2buf(hex) {
  const [a, b] = hex.split('/');
  return Buffer.from(a.padStart(8, '0') + b.padStart(8, '0'), 'hex');
}

export function lsn_buf2u64(buf) {
  return BigInt('0x' + buf.toString('hex'));
}

export function lsn_u64_2buf(u64) {
