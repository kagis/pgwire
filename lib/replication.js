...
class ReplicationStream extends Readable {
  constructor({ rx, tx, startLsn, ackIntervalMillis }) {
    ...
    if (ackIntervalMillis > 0) {
      // TODO .ack will not work without timer
  ...
  ackImmediate(lsn) {
    ...
    if (this._tx.writableHighWaterMark > this._tx.writableLength) {
      return this._tx.write(updateStatusMessage(incLsn(lsn)));
    }
  }
  end() {
    this.ackImmediate();
...
function readLsn(bufr) {
  const h = bufr.readUInt32BE();
  const l = bufr.readUInt32BE();
