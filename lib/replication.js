...
function logicalReplication(client, {
  slot,
  startLsn, // todo validate lsn format
  ackIntervalMs,
  options = {},
}) {
  return client.session(async conn => {
...
class PgoutputDecoder extends Transform {
  ...
  _handleRelation({ lsn, endLsn, time }, bufr) {
    ...
    this._attrs[rel.relationid] = rel.attrs.map(att => {
      const { decodeText } = datatypes[att.typeid] || datatypes.noop;
...
function readTuple(bufr, attrs) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    switch (bufr.read_u8()) {
      case 0x74: // 't'
        const raw_val = bufr.read_countedstr();
        tuple[name] = decode(raw_val);
        break;
      case 0x6e: // 'n'
        tuple[name] = null;
        break;
