...
class Connection extends EventEmmiter {
  ...
  query(optionsOrScript) {
    if (typeof optionsOrScript == 'string') {
      return this._querySimple({ script: optionsOrScript });
    }
    if (optionsOrScript.extended) {
      return this._queryExtended(optionsOrScript);
    }
    return this._querySimple(optionsOrScript);
  }
  ...
  _connectIfNotConnected() {
    ...
    const socket = net.connect(unixSocket || { host: hostname, port });
  ...
  _passResponse(chunk) {
    return this._responses[0].push(chunk);
  }
  ...
  _request(messages) {
    ...
    if (!this._tx) {
      return response.destroy(Error('Query after connection end'));
    }
    clearTimeout(this._idleTimer);
  ...
  _recvAuthenticationSASL(_msg) {
    const clientNonceB64 = randomBytes(24).toString('base64');
...
class Response extends Readable {
  ...
  _fetch() {
    if (!this._promise) {
      this._promise = fetchResponse(this);
    }
