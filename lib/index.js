...
const config = url2conf(process.env.POSTGRES || 'postgres://postgres@127.0.0.1:5432/postgres');
...
class Oneshot extends ClientBase {
  ...
  _request(messages) {
    const conn = new Connection(this._connOptions);
    try {
      const response = conn._request(messages);
      conn.on('error', err => response.emit('error', err));
      return response;
    } finally {
      conn.end();
    }
...
class Connection extends ClientBase {
  constructor(options = config) {
    ...
    this._request([new fe.StartupMessage(parameters)])
    .on('end', _ => this.emit('ready'))
    ...
    if (lastResponse) {
      ...
      // so we need to wait for all pending responses
      this._tx.write(new fe._Block(new Promise(resolve => {
        lastResponse.on('readyForQuery', resolve);
      })));
    }
  ...
  _onFinished(err) {
    ...
    while (this._responses.length) {
      const response = this._responses.shift();
      setImmediate(_ => response.emit('error', response._pgerror || err || Error('Connection terminated')));
    }
  }
  _request(messages) {
  ...
  _recvError(msg) {
    ...
    const err = Object.assign(Error(), props, {
      name: 'PGError',
      code: 'PGERR_' + code,
    });
    const [response] = this._responses;
    if (!response) {
      return this.emit('error', err);
    }
    response.push(msg);
    response._pgerror = Object.assign(err, {
      prevError: response._pgerror,
    });
  }
  ...
  _recvNotice(msg) {
    if (this._responses[0] && this._responses[0].emit('notice', msg)) {
      return;
    }
...
class Pool extends ClientBase {
  constructor(options) {
    ...
    for (let i = 0; i < 4; i++) {
      const conn = new Connection(options);
      conn.on('error', this.emit('error', err));
